...
class DSSM_VAE():
    ...
    def __init__(self, nb_words, max_len, embedding_matrix, dim, optimizer=Adam(), kl_rate=0.01, keep_rate_word_dropout=0.5, enableKL=True, enableCond=False, mode=1, enableGRU=True, enableSemi=False):
    ...
    def build(self):
        ...
        if self.enableGRU:
            encoder_lstm = Bidirectional(GRU(self.hidden_dim, name="q_gru"))
            doc_encoder_lstm = Bidirectional(GRU(self.hidden_dim, name="d_gru"))
        else:
            encoder_lstm = GlobalMaxPooling1D()
            doc_encoder_lstm = GlobalMaxPooling1D()
        ...
        self.doc_decoder_dense = Dense(self.nb_words, activation='softmax', name="d_rec")
