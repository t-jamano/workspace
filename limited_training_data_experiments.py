...
def parse_args():
    ...
    parser.add_argument('--limit', type=int, default=10000, help='Size of Labelled data')
...
if __name__ == '__main__':
	...
	if model in ["dssm", "dssm_hybrid", "dssm_max"]:
		if "QQ" in train_data:
	...
	model_name = "%s_m%d_%s_limit%d_%s_h%d_l%d_b%d" % (run.name(), mode, train_data, limit, date_time, hidden_dim, latent_dim, batch_size)
	...
	# q_dec_inputs = np.load('%sdata/train_data/%s.q.di.npy' % (path,train_data))[:100]
	# q_dec_outputs = np.load('%sdata/train_data/%s.q.do.npy' % (path,train_data))[:100]

	# d_enc_inputs = np.load('%sdata/train_data/%s.d.npy' % (path,train_data))[:100]
	# d_dec_inputs = np.load('%sdata/train_data/%s.d.di.npy' % (path,train_data))[:100]
	# d_dec_outputs = np.load('%sdata/train_data/%s.d.do.npy' % (path,train_data))[:100]
